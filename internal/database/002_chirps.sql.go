...
const getChirpsWithAuthorId = `-- name: GetChirpsWithAuthorId :many
SELECT id, created_at, updated_at, body, user_id from chirps
WHERE user_id = $1
ORDER BY created_at ASC
`

func (q *Queries) GetChirpsWithAuthorId(ctx context.Context, userID uuid.UUID) ([]Chirp, error) {
	rows, err := q.db.QueryContext(ctx, getChirpsWithAuthorId, userID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
